...
def figure_2(key,ref_file):
    ...
    bar = Bar('Processing...',max=len(ids)-2)
    ...
    for id in ids: 
        ...
        r_jds = r_mjds+2400000.5
        r_mags = np.array(r_file['mag'])
        r_magerrs = np.array(r_file['magerr'])
        q = quas_per(JD=r_jds,mag=r_mags,err=r_magerrs,ref_file=ref_file)
        if q != 'NaN':
            m = codyM(r_mags)
            ms.append(m)
            qs.append(q)
...
figure_2(key='/home/thaddaeus/FMU/HRL/LAH2.0/data/695/no_clusters/key.csv',ref_file='/home/thaddaeus/FMU/HRL/LAH2.0/efforts/beta/mean_mags_and_stds.csv')