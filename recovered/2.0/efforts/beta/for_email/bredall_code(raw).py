...
def quas_per(JD: np.array, mag: np.array, err: np.array) -> float:
...
def flux_asym(mag: np.array) -> float:
    r"""Calculate flux-asymmetry M for a given lightcurve.
    ----------
    mag : np.array
        The magnitude measurements for the lightcurve
    Returns
    -------
    out : float
        The flux-asymmetry of the lightcurve, with M<0 => more brightening
        events, M>0 => more dimming events, and M=0 => symmetric.
    """
    # Grab the upper and lower 10% of measurements
    upper_lower = mag[
        np.logical_or(mag > np.quantile(mag, .90),
                      mag < np.quantile(mag, .10))
    ]

    # Calculate :)
    flux_asym = (np.mean(upper_lower) - np.median(mag)) / np.std(mag)
