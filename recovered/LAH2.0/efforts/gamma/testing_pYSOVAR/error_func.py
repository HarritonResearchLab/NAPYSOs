def first_test(slope,slope_error):
    #Import(s)
    import math
    import numpy as np
    ...
    alpha_plus  = math.asin((slope+slope_error)/np.sqrt((slope+slope_error)**2 + 1**2))
    alpha_minus = math.asin((slope-slope_error)/np.sqrt((slope-slope_error)**2 + 1**2))
    sd_alpha = 0.5*( np.abs(alpha - alpha_plus) + np.abs(alpha - alpha_minus) ) 
    return math.degrees(alpha), math.degrees(sd_alpha)

def cmd_slope_simple(data1, data2, data1_error, data2_error):
